...
export const HeroTemplate = (args) => (
  <Hero variant={args.variant} as={args.as}>
    ...
    <Paragraph>A cyberpunk tech interface component library for React</Paragraph>
    <Cluster>
...
<Canvas>
  <Story name="Hero Split">
    <Hero variant="split">
      <Box>
        <Stack variant="tight">
          <Heading level={2}>Build Cyberpunk Interfaces</Heading>
          <Paragraph>
      ...
      <Card>
        <CardContent>
          <Paragraph variant="small">Visual content or demo goes here</Paragraph>
        </CardContent>
      </Card>
    </Hero>
  </Story>
